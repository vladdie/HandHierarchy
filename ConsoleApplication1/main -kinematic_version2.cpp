...
void init()
{
	...
	const float radiusTop = 3.0;
	const float radiusBottom = 3.0;
	...
	cylinder = new Cylinder(1.5, 0.5, 0.5, colorTop, colorBottom, 16);
...
void firstVision()
{
	...
	glm::mat4 child1Local = glm::translate(Model, glm::vec3(0,2,0));//1: height of the parent(cube)
...
void renderSkeleton(Node* bone)
{
	...
	if(bone->m_nodeName == "rootBone")
	{
		//std::list<Node*> temp = bone->m_listChildren;
		glm::mat4 nodeGlobal = bone->getGlobalTransform();
		nodeGlobal =  glm::rotate(glm::mat4(1.0), theta, glm::vec3(0,0,1)) * nodeGlobal;
	...
	else
	{
		//std::list<Node*> temp = bone->m_listChildren;
	 //get animation
	    bone->nodeLocalMatrix = glm::rotate(bone->nodeLocalMatrix,theta, glm::vec3(0,0,1)) ;
		 
		glm::mat4 nodeGlobal = bone->m_parent->nodeGlobalMatrix * bone->nodeLocalMatrix;
	   glUniformMatrix4fv(modelMatrixID, 1, GL_FALSE, &nodeGlobal[0][0]);
	   cylinder->draw();
...
void display(){
	...
	std::list<Node*>::iterator it;
     for(it = Skeleton->allNodes.begin() ; it!=Skeleton->allNodes.end() ; it++)
     {
          renderSkeleton((*it));
	 }
	//  
	////glm::mat4 nodeLocalMatrix =  glm::rotate(glm::mat4(1.0), (float)90.0, glm::vec3(0,0,1));
	// glm::mat4 nodeLocalMatrix= glm::mat4(1.0);
	//   nodeLocalMatrix = glm::translate(glm::mat4(1.0), glm::vec3(0,5,-3))*nodeLocalMatrix;
	//	//get animation
	//   phi+=0.1;
	// //   nodeLocalMatrix = glm::rotate(nodeLocalMatrix,phi, glm::vec3(0,0,1)) ;
	//	glm::mat4 nodeGlobal = nodeLocalMatrix;
	//	//glm::mat4 nodeGlobal = Skeleton->rootBone->getGlobalTransform()*nodeLocalMatrix;
	//   glUniformMatrix4fv(modelMatrixID, 1, GL_FALSE, &nodeGlobal[0][0]);
	//   cylinder->draw();
}
